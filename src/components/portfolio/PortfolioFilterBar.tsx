...
interface PortfolioFilterBarProps {
  ...
  sortOptions?: { id?: string | null; label?: string | null }[] | null;
}
...
export default function PortfolioFilterBar({
  ...
  const visibleSortOptions = (sortOptions?.length ? sortOptions : defaultSortOptions).filter((opt) => opt.id && opt.label);
      ...
      <div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
          <div className="flex-1">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#9aa7b9]">{filterHeading || "Categories"}</p>
          ...
          <div className="w-full lg:max-w-xs">
            <p className="text-xs font-semibold uppercase tracking-[0.22em] text-[#9aa7b9]">{sortHeading || "Sort by"}</p>
            <select
              value={selectedSort}
